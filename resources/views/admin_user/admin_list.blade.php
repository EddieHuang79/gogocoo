		...
		<tbody>
			@foreach($user as $row)
				<tr>
					...
					<td>{{ $row->deadline }}</td>
					<td>
						<input type="button" value="{{ $txt['edit'] }}" onClick="location.href='/user/{{ $row->id }}/edit?';"/>
						<input type="button" value="{{ $txt['extend_deadline'] }}" class="extend_account_deadline" account="{{ $row->account }}" userId="{{ $row->id }}"/>
					</td>
				</tr>
			@endforeach													
		</tbody>
